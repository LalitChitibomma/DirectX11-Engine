...
bool CTexture::Initialize(ID3D11Device* device, ID3D11DeviceContext* deviceContext, char* filename)
{
	...
	result1 = LoadTarga32Bit(filename);
	result2 = LoadTarga24Bit(filename);
	if(!result1 && !result2)
	{
		return false;
	}
...
bool CTexture::LoadTarga32Bit(char* filename)
{
...
bool CTexture::LoadTarga24Bit(char* filename)
{
	int error, bpp, imageSize, index, i, j, k;
	FILE* filePtr;
	unsigned int count;
	...
	// Calculate the size of the 24 bit image data. (8 bits = 1 byte)
